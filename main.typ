...
#show: ams-article.with(
  ...
  abstract: [Here is a template for writing AMS style documents using Typst. At the end, I also appended a cheat sheet for typing math symbols.]
)


= Introduction

Typst is a new document typesetting system with many features. Its syntax draws heavily on Markdown and it is also designed for conveniently typesetting mathematical equations. 

= Basic Syntax

The Typst syntax is designed to be simple and easy to read when it is unrendered. It is heavily inspired by Markdown, but it also has new syntax for mathematical equations and some extra features.
- Bold text: `*lorem ipsum*` renders *lorem ipsum*
- Italic text: `_lorem ipsum_` renders _lorem ipsum_. 
- Literal text: \``lorem ipsum`\` renders `lorem ipsum` (note that any special characters inside grave accents are not rendered)
- Hyperlink text: `#link("www.google.com")[Example URL]` renders #link("www.google.com")[Example URL].

Typst can also render lists. 
- Unordered list: begin consecutive lines with dash `-`
- Ordered list: begin consecutive lines with dash `+`
  + Create nested lists by indenting 
  + Customize using the `#enum(...)` function. For example, `#enum(numbering: "(I)")` creates roman numerals: 
    #enum(numbering: "(I)")[Numeral][Numeral]

= Document Structure <doc_structure>

Begin a major section by typing `= Section Title` on an isolated line. This is the Typst equivalent of `\section{Section Title}` in LaTeX. To begin a subsection, type `== Subsection Title` on an isolated line. 
...
Use global document settings to control how subsections are numbered, as described in the #link("https://typst.app/docs/reference/model/heading/")[Headings] documentation.

== References

It is possible to reference document objects like sections, subsections, figures, equations, and citations. Label an object by adding `<label_name>` after its definition. The placement is similar to LaTeX `\label{...}`, but check the docs for specifics. Reference the object later using `@label_name`. For example, @doc_structure has the label `@doc_structure`.


= Mathematics 

Like LaTeX, you can render inline equations with `$...$`. To write a multiline equation, just use the same syntax on an isolated line: 

$
1/sqrt(2pi) integral_(-oo)^(oo) e^(-x^2/2) d x = 1
$

In any multiline equation, use `\` to create a line break and use `&` to control vertical alignment, similar to the `\align{...}` LaTeX environment. 
$
(integral_(-oo)^(oo) e^(-x^2) d x)^2 & = (integral_(-oo)^(oo) e^(-(x^2 + y^2)) thin d x thin d y ) \ 
& = 2 pi integral_(0)^(oo) e^(-r^2) r thin d r \ 
& = pi
$
The left hand size is written as `(integral_(-oo)^(oo) e^(-x^2) d x)^2`. 
Typst can tell the difference between symbol grouping parenthesis, like the exponent `e^(-x^2)`, and mathematical parenthesis, like those surrounding the integral. It automatically matches and resizes mathematical parenthesis for their content. It also automatically renders fractions `$(numerator...)/(denominator...)$`, such as
$
(1 + 3 + 5 + 9 + dots)/(2 + 4 + 6 + 8 + dots)
$

and inline fractions can be written as `$a\/b$`, which renders $a\/b$. 

Finally, some additional resources on typesetting math: 
- #link("https://typst.app/docs/reference/math/")[Typst Math Documentation]: the primary documentation.
- #link("https://github.com/johanvx/typst-undergradmath")[UndergradMath Reference]: a good cheat sheet for math commands, which I've also copied at the end of this document. 

== Equation Numbering

At the time of writing, Typst only natively supports _global_ equation numbering (every equation gets a number). I wrote a special function `#eq[...]`, which is included in `template.typ`, which is ok but still not perfect (see the source for commentary). 

#eq[
$ bb(E)_(Z thin ~ thin cal(N)(0, I_d))[norm(Z)] & = bb(E)[(norm(Z)^2)^(1/2)] \
 & <= bb(E)[norm(Z)^2]^(1/2) $ 
$ & = sqrt(d) $ <linetwo>
]

/*
inside #eq[...], you may write multiple $...$ and each one gets a number and shares the same alignment. I am not sure how to enable/disable numbering on a line-by-line basis. In the above example, the first two line share the number (1). 
*/

= References, Figures, and other external files

The current template includes a file `refs.bib`. Any BibTeX citations in this file are imported to the main document by the command `#bibliography("refs.bib")` which appears at the end of `main.typ`. This command also renders the bibliography. By default, citations are numbered (eg. @knuth97). 
...
#align(center, table(
  ...
  [$\u{224D}$], [`\u{224D}`], [$tilde.eq$], [`tilde.eq`], [$n!$], [`n!`],
  ...
  [$prop$], [`prop`], [$\u{2298}$], [`\u{2298}`], [$sqrt("")$], [`sqrt("")`],
  [$\u{2250}$], [`\u{2250}`], [$harpoon.tr$], [`harpoon.tr`], [$checkmark$], [`checkmark`],
))
